...
public class CustomBoard extends Board implements Cloneable{  
    ...
    public CustomBoard(BoardFrame boardFrame, Menu menu) {
        ...
        Txt.setText("<html> <center> Click a piece, once selected click a square <br/> on the board to place <br/> Click the piece again to de-select the piece <br/> Press delete on selected piece to remove piece</center> </html>");
            ...
            if (mouse_X > image.getRect().getX() && mouse_X < image.getRect().getX() + image.getWidth()
                ...
                if (image.getFilePath() == play_button_file_path) {
                    ...
                    for (Piece piece: All_Pieces){
                        ...
                        if (piece.getClass()==King.class && piece.moveIsOutOfBounds(piece.getX(),piece.getY())==false && piece.isWhite()){
                            whiteKingPlaced=whiteKingPlaced+1;
                        }
                        if (piece.getClass()==King.class && piece.moveIsOutOfBounds(piece.getX(),piece.getY())==false && piece.isWhite()==false){
                ...
                if (image.getFilePath() == delete_button_file_path) {
                    ...
                    if (Active_Piece!=null && !(Active_Piece.moveIsOutOfBounds(Active_Piece.getX(),Active_Piece.getY()))){
